...
public class RecommendService {
    ...
    private void computeStatistics(Seats seats) {
        ...
        for(int i = 0; i < seats.getSeats().size() ; i ++) {
            for(int j = 0; j< seats.getSeats().get(i).getContestions().size() ; j++){
                if (list.contains(j+1)){
                    congestionStatistics.add(
                            new CongestionStatistics(
                                    j+1,
                                    seats.getSeats().get(i).getContestions().get(j),
                                    seats.getSeats().get(i).getContestions().get(compareCar),
                                    compareCar));
                    log.info("seat = {}", seats.getSeats().get(i).getContestions().get(compareCar));
                }
            }
        }
